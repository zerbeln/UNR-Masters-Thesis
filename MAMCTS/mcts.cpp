...
void monte_carlo::decode(int p, vector<int> vin){ //Decodes the rollout policy from a binary string
    num = 0;
    for(int i = 0; i < 3; i++){
        num += vin.at(p+i)*pow(2,i);
    }
}

void monte_carlo::assign_rollout_policy(vector <int> vin){
    for(int rp = 0; rp < rollout_steps; rp++){ //Assign rollout policy
        rollout_policy.push_back(0);
        decode(3*rp, vin);
        rollout_policy.at(rp) = (int)round(num*(4/(pow(2,3)-1)));
    }
}
...
void monte_carlo::rollout(multi_tree *tp, multi_agent *map, int n){
    ...
    for(int i = 0; i < rollout_steps; i++){
        if(rand_rollout == true){
            act = rand() % 4; //Rollout occurs according to defined policy from GA
        }
        if(rand_rollout == false){
